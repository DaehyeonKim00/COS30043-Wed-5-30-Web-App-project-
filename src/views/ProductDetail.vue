...
export default {
  ...
  setup() {
    // Using composables for auth and cart state management (advanced feature - tuan added)
    const {
      user,
      requireAuth,
      showAuthModal,
  ...
  methods: {
      ...
      getProductById(productId)
        .then((data) => {
          ...
          if (self.userFromStore) {
            getWishlist(self.userFromStore.id).then((items) => {
              self.inWishlist = items.some(
          ...
          getRecommendedProducts(data.category, productId).then((products) => {
    ...
    addToCartHandler() {
      // Using useCart composable for cart management (advanced feature - tuan added)
      var self = this;
      ...
      self
        .addItem(self.userFromStore.id, self.product.id, 1)
        .then((data) => {
          if (data.success) {
